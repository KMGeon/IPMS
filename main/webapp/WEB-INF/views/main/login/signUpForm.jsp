...
">
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="selectedBox" class="select2 form-control select2-hidden-accessible"
                                            multiple data-select2-id="12" tabindex="-1" aria-hidden="true"
                                            onchange="handleOnChange(this)">
                                        <optgroup label="보유한 기술 스택을 선택하세요.">
                                            <c:forEach var="item" items="${list}" varStatus="idx">
                                                ${idx.count}
                                                <option value="${item.commonCodeValue}"
                                                        name="techStackVOList[${idx.count-1}].techStackCode">${item.commonCodeValue}</option>
                                            </c:forEach>
                                            <input type="hidden" id="techCode" name="techStackVOList[0].techStackCode"
                                                   value=""/>
                                        </optgroup>
                                    </select>
                                </td>
