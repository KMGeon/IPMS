                        ...
                        <c:forEach var="item" items="${checkMyProject}">
                            <c:if test="${item.memCode ne mvo.member.memCode}">
                                <button class="btn btn-outline-secondary round" style="width:110px;"
                                    ...
                                    <i class="fa fa-user-o" style="color:#02b5b8;"></i>&nbsp;&nbsp;가입 신청
                                </button>
                            </c:if>
                        </c:forEach>&nbsp;&nbsp;&nbsp;&nbsp;
                        <button id="markBtn" class="btn btn-outline-secondary round" style="width:110px;">
                            <i class="feather icon-heart" style="color:red;"></i>&nbsp;&nbsp;북마크
                        </button>
                    </div>
                    <!-- 						</div> -->
                ...
                <div class="row">
                    ...
                    <div class='col-sm-2'>
                        <button type="button" class="btn btn-secondary"
                                style="width: 150px;" id="projCmtAdd">댓글 등록
                        </button>
                    </div>
                </div>
                <input type="hidden" class="form-control" id="projBdCmtId" name="projBdCmtId"
                       value="${ProjSmryCmtVO.projSmryCmtNum}">
                                    ...
                                    <div>
                                        <div style='padding-bottom: 5px;'>
                                            <div style='font-size: 15px; color:#02b5b8;'>
												...
												<fmt:formatDate value="${ProjSmryCmtVO.projSmryCmtWriteDate}"
                                        ...
                                        <div id="cmtCts" style="display:flex; align-items:center;">
                                            ...
                                            <div id="ctsInput${ProjSmryCmtVO.projSmryCmtNum}"
                                                 style="padding-top:7px; padding-bottom:7px; display:none; width:450px;">
                                                <div style="display:inline-block; align-items:center;">
                                                    <input type="text" class="form-control"
                                                           id="lowCmtInsert${ProjSmryCmtVO.projSmryCmtNum}"
                                                           name="projSmryCmtCts" placeholder="댓글을 입력하세요."
                                                           style="width:300px;">
                                                </div>
                                                <div style="display:inline-block; align-items:center;">
                                                    <button onclick="fn_lowCmtInsertOK('${ProjSmryCmtVO.projSmryCmtNum}')"
                                            ...
                                            <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
												...
												<!-- 로그인한 회원과 댓글 작성자가 일치하면 댓글 수정,삭제 버튼 -->
													<div style="padding-left:20px;"> 
														<a id="lowCmtOpen${ProjSmryCmtVO.projSmryCmtNum}"
                                                           style="font-size: 12px; text-decoration: underline;"
                                                           onclick="fn_lowCmtInsert('${ProjSmryCmtVO.projSmryCmtNum}')"
                                                           type="button">댓글작성</a>&nbsp;&nbsp;
													<c:if test="${ProjSmryCmtVO.memEmail eq mvo.member.memEmail}">
                                                        <a style='color: red; font-size: 12px; text-decoration: underline; padding-right:5px;'
                                                           onclick='fn_delete(${ProjSmryCmtVO.projSmryCmtNum})'
                                                           type='button'>삭제</a>
                                                        <a style='color: grey; font-size: 12px; text-decoration: underline;'
                                                           id='cmtModify' type='button'
                                                           onclick="fn_update('${ProjSmryCmtVO.projSmryCmtNum}','${stat.index}')">수정</a>
                                                    </c:if>
													</div>
												</span>
                                                <span id="fnUpdate${ProjSmryCmtVO.projSmryCmtNum}"
                                                      style="display:none;">
													<input type="text" class="form-control"
                                                           id="projSmryCmtCts${ProjSmryCmtVO.projSmryCmtNum}"
                                                           name="projSmryCmtCts" value="${ProjSmryCmtVO.projSmryCmtCts}"
                                                           style="display: none; width: 500px; padding-bottom:5px;"/>
													<button type="button" class="btn btn-warning btn-sm"
                                                            onclick="fn_updateCancel(${ProjSmryCmtVO.projSmryCmtNum}, '${stat.index}')"
                                                            style="width:50px; float:right; margin:3px;">취소</button>
													<button type="button" class="btn btn-secondary btn-sm"
                                                            onclick="fn_updateGo(${ProjSmryCmtVO.projSmryCmtNum}, '${stat.index}')"
                                                            style="width:50px; float:right; margin:3px;">등록</button>
												</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <input type="hidden" class="form-control" id="projSmryCmtNum" name="projSmryCmtNum"
                                       value="${ProjSmryCmtVO.projSmryCmtNum}">
                                <input type="hidden" id="writer" name="writer"
                                       value="<sec:authentication property='principal.member.memCode'/>"/>
                            </form>
                        </div>
                        <!-- ------------------------------------------------ -->
                    </c:if>
                                        ...
                                        <div style="padding-bottom: 5px;" id="${ProjSmryCmtVO2.projSmryCmtNum}">
                                            ...
                                            <div style="font-size: 15px; color:#3B4897;">
                                                <i class="feather icon-corner-down-right"></i><i
                                                    ...
                                                    style="font-size: 8px; color: grey; padding-left:5px;"><fmt:formatDate
                                                    value="${ProjSmryCmtVO2.projSmryCmtWriteDate}"
                                                    pattern="yyyy-MM-dd HH:mm"/></span>
                                            </div>
                                        </div>
                                        <div id="cmtCts2" style="display: flex;">
                                            <div style="display: flex;" id="lowCts${ProjSmryCmtVO2.projSmryCmtNum}">
                                                &nbsp;&nbsp;&nbsp;&nbsp;${ProjSmryCmtVO2.projSmryCmtCts}&nbsp;&nbsp;
                                            </div>
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input style="width: 400px;" type="hidden"
                                                   value="${ProjSmryCmtVO2.projSmryCmtCts}">
                                            <span id="fnEdit${ProjSmryCmtVO2.projSmryCmtNum}">
													<c:if test="${ProjSmryCmtVO2.memEmail eq mvo.member.memEmail}">
                                                        <a style="color: red; font-size: 12px; text-decoration: underline;"
                                                           onclick="fn_delete('${ProjSmryCmtVO2.projSmryCmtNum}')"
                                                           type="button">삭제</a>&nbsp;
                                                        <a style="color: grey; font-size: 12px; text-decoration: underline;"
                                                           onclick="fn_update2('${ProjSmryCmtVO2.projSmryCmtNum}','${stat.index}')"
                                                           type="button">수정</a>
                                                    </c:if>
												</span>
                                            <span id="fnUpdate${ProjSmryCmtVO2.projSmryCmtNum}" style="display:none;">
													<input type="text" class="form-control"
                                                           id="projSmryCmtCts${ProjSmryCmtVO2.projSmryCmtNum}"
                                                           name="projSmryCmtCts"
                                                           value="${ProjSmryCmtVO2.projSmryCmtCts}"
                                                           style="display: none; width: 500px; padding-bottom:5px;"/>
													<a class="btn btn-warning btn-sm"
                                                       onclick="fn_updateCancel2(${ProjSmryCmtVO2.projSmryCmtNum}, '${stat.index}')"
                                                       style="color:white; width:50px; float:right; margin:3px;">취소</a>
													<a class="btn btn-secondary btn-sm"
                                                       onclick="fn_updateGo(${ProjSmryCmtVO2.projSmryCmtNum}, '${stat.index}')"
                                                       style="color:white; width:50px; float:right; margin:3px;">등록</a>
												</span>
                                        </div>
                                    </div>
                                    <br>
                                    <input type="hidden" class="form-control" id="projSmryCmtNum" name="projSmryCmtNum"
                                           value="${ProjSmryCmtVO2.projSmryCmtNum}">
                                    <input type="hidden" id="writer" name="writer"
                                           value="<sec:authentication property='principal.member.memCode'/>"/>
                                </form>
                            </div>
...
// 		alert("댓글 등록 떠라");
        ...
        let data = {
            "projId": projId,
            "projSmryCmtNum": projSmryCmtNum,
            "memCode": memCode,
            "projSmryCmtCts": projSmryCmtCts
        };
